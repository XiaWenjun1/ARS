"""
CartPole Continual Learning Training Script
"""
...
def main():
    ...
    metrics_summary = trainer.metrics.get_core_metrics_summary(
        ...
        convergence_threshold=getattr(config, 'CONVERGENCE_THRESHOLD', 195)
    )
    ...
    generate_forgetting_matrix_plot(cf_matrix, env.total_tasks, "CartPole")
...
def generate_core_metrics_plots(convergence_data, task_rewards, total_tasks):
    ...
    plt.tight_layout()
    plt.savefig('visualizations/task2_core_metrics_cartpole.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("Core metrics plots saved to: visualizations/task2_core_metrics.png")
