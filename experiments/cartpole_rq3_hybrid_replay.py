...
def main(seeds: List[int] = [1, 2, 3], episodes_per_task: int = 100, cycles: int = 2):
    ...
    for condition in conditions:
        ...
        for seed in seeds:
            ...
            print(f"  ✅ Seed {seed} completed: {elapsed:.1f}s, perf={result['avg_performance']:.1f}")
            
            # Save each seed result immediately
            with open(f'temp_results_{condition}_seed{seed}.json', 'w') as f:
                json.dump({
                    'seed': seed,
                    'condition': condition,
    ...
    # Generate comprehensive visualizations using the new module
    print("\n📊 Generating comprehensive visualizations...")
    create_comprehensive_analysis(all_results, save_dir="./visualizations/rq3")
